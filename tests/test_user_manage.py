...
def test_update_username(client):
    ...
    r = client.post("/users/me/update-name", params={"new_username": "李四"}, headers=headers)
...
def test_update_password(client):
    """改密码后旧密码登录失败，新密码登录成功。"""
    headers = _register_and_login(client)
    r = client.post("/users/me/update-password", params={"new_password": "654321"}, headers=headers)
    assert r.status_code == 200
    # 旧密码登录失败
    assert client.post("/login", json={"username": "张三", "password": "123456"}).status_code == 401
    # 新密码登录成功
    assert client.post("/login", json={"username": "张三", "password": "654321"}).status_code == 200
...
def test_update_name_requires_auth(client):
    """未登录改名字 → 401。"""
    r = client.post("/users/me/update-name", params={"new_username": "李四"})
